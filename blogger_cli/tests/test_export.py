...
class TestBasic(unittest.TestCase):
    ...
    def test_blog_config(self):
        ...
        result = self.runner.invoke(
            cli, ["export", "blog_config", "-o", self.export_dir]
        )
        self.assertEqual(result.exit_code, 0)
        self.assertEqual(["test1.json"], os.listdir(self.export_dir))
    ...
    def test_blog_index(self):
        ...
        result = self.runner.invoke(cli, ["export", "blog_index", "-o", "blog"])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_design_assets(self):
        ...
        self.assertEqual(result.exit_code, 0)
        expected_files = {"css"}
        self.assertEqual(expected_files, set(os.listdir(self.export_dir)))

        result = self.runner.invoke(cli, ["setdefault", "test1"])
        result = self.runner.invoke(cli, ["export", "design_assets", "-o", "assets"])
        self.assertEqual(result.exit_code, 0)
    ...
    def test_export_blog_layout(self):
        ...
        result = self.runner.invoke(cli, ["setdefault", "test1"])
        result = self.runner.invoke(cli, ["export", "blog_layout", "-o", "assets"])
        self.assertEqual(result.exit_code, 0)
        expected_files = {
            "assets",
            "blog",
    ...
    def test_export_blog_template(self):
        ...
        result = self.runner.invoke(cli, ["setdefault", "test1"])
        result = self.runner.invoke(cli, ["export", "blog_template", "-o", "assets"])
        self.assertEqual(result.exit_code, 0)
        expected_files = {
            "css.html",
            "dark_theme.html",
