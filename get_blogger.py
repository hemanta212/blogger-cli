...
It does, in order:
  ...
  - Downloads all its dependencies in the blogger_cli/_vendor directory.
...
HOME = expanduser("~")
BLOGGER_CLI_HOME = os.path.join(HOME, 'local', ".blogger_cli")
BLOGGER_CLI_BIN = os.path.join(BLOGGER_CLI_HOME, "bin")
BLOGGER_CLI_ENV = os.path.join(BLOGGER_CLI_HOME, "env")
BLOGGER_CLI_LIB = os.path.join(BLOGGER_CLI_HOME, "lib")
BLOGGER_CLI_LIB_BACKUP = os.path.join(BLOGGER_CLI_HOME, "lib-backup")
...
POST_MESSAGE_UNIX = """{blogger-cli} ({version}) is installed now. Great!
...
POST_MESSAGE_WINDOWS = """{blogger-cli} ({version}) is installed now. Great!
...
POST_MESSAGE_UNIX_NO_MODIFY_PATH = """{blogger-cli} ({version}) is installed now. Great!
To get started you need {blogger-cli}'s bin directory ({blogger_cli_home_bin}) in your `PATH`
...
POST_MESSAGE_WINDOWS_NO_MODIFY_PATH = """{blogger-cli} ({version}) is installed now. Great!
...
class Installer:
    ...
    METADATA_URL = "https://test.pypi.org/pypi/blogger-cli/json"
    ...
    BASE_URL = "https://github.com/hemanta212/blogger-cli/releases/download/"
    ...
    def get_version(self):
        print(colorize("info", "Retrieving Blogger-cli metadata"))
        ...
        if os.path.exists(BLOGGER_CLI_LIB):
            with open(os.path.join(BLOGGER_CLI_LIB, "blogger_cli", "__version__.py")) as f:
            ...
            if not current_version_re:
                print(
                    colorize(
                        "warning",
                        "Unable to get the current Blogger-cli version. Assuming None",
                    )
                )
            else:
    ...
    def ensure_home(self):
        ...
        if not os.path.exists(BLOGGER_CLI_HOME):
            os.mkdir(BLOGGER_CLI_HOME, 0o755)
    ...
    def make_lib(self, version):
        ...
        """
        if os.path.exists(BLOGGER_CLI_LIB_BACKUP):
            shutil.rmtree(BLOGGER_CLI_LIB_BACKUP)

        # Backup the current installation
        if os.path.exists(BLOGGER_CLI_LIB):
            shutil.copytree(BLOGGER_CLI_LIB, BLOGGER_CLI_LIB_BACKUP)
            shutil.rmtree(BLOGGER_CLI_LIB)

        try:
            self._make_lib(version)
        except Exception:
            if not os.path.exists(BLOGGER_CLI_LIB_BACKUP):
                raise

            shutil.copytree(BLOGGER_CLI_LIB_BACKUP, BLOGGER_CLI_LIB)
            shutil.rmtree(BLOGGER_CLI_LIB_BACKUP)
    ...
    def _make_lib(self, version):
        ...
        name = "blogger-cli-{}.tar.gz".format(version)
        ...
        with temporary_directory(prefix="blogger-cli-installer-") as dir_:
...
def main():
    ...
    installer = Installer(
        ...
        preview=args.preview or string_to_bool(os.getenv("BLOGGER_CLI_PREVIEW", "0")),
        force=args.force,
